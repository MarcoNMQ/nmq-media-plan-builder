...
import { BENCH_FIELDS, BENCH_HELP, BENCH_IS_PCT, channelKeyFor } from '@/lib/constants';
import { useMediaPlanStore } from '@/lib/store';
import type { Channel, ChannelConfig, Goal } from '@/lib/types';
...
export function BenchmarkEditor({
  ...
  async function applyAiPreset(preset: typeof PRESETS[number]) {
    ...
    setAiError(null);
    try {
      const res = await fetch('/api/ai-bench-preset', {
        method: 'POST',
